...
- 回复续发：缓存最近一次行动的结果、选项和已生成图片，可用 `/故事 继续` 完整重发。
...
每轮继续故事时，前三项由圆桌生成。无人交谈时追加第 4 项自由行动说明；有人交谈时固定追加 `4. 杀害`、`5. 侵犯` 和第 6 项自由行动说明。
...
`/圆桌会议` 只展示模型讨论，不单独展示开局底稿。开局、加入角色或故事行动产生的新讨论会覆盖旧记录，仅保留最近一次圆桌调用，且不会写入长期故事记忆。

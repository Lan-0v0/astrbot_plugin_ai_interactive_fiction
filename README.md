...
- 普通聊天与游玩消息共存：只有进入故事或待选故事状态后才启用自然语言判断，未开局消息完全不调用插件判断模型。
...
“启用自然语言”只在玩家已经进入故事，或已经通过 `/故事 开始` 进入待选故事状态后生效。未开局时普通聊天不会调用全局判断 LLM；开始和加入请使用明确指令。进入故事后，可直接 @机器人表达结束、存读档、查看圆桌和实际行动。

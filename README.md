...
- 分段开局：依次发送玩家自身信息、视野所及环境和人物立绘，不提前解释隐藏世界观或剧情。
...
推送符合语义化版本的标签（例如 `v0.0.4`）后，GitHub Actions 会运行测试、检查版本一致性、构建小于 16 MB 的插件压缩包并创建 GitHub Release。详细步骤见 [发布指南](docs/RELEASING.md)。
